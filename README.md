...
- `assignments/practice_activities/week_3/`
  - `joining_and_merging/` — `pandas.html` (**Pandas in motion**, an interactive Learn + Quiz page served by GitHub Pages), the Topic 3.1 companion notebook (generated by `tools/build_week3_joins.py`), and PA 3.1 (Concatenating, Joining, and Pivoting; MovieLens data). `week_3/README.md` has the website test checklist
  - `distances_between_observations/` — Topic 3.2 (15-minute opener: types of distance, why, when; built by `tools/build_week3_distances.py`) and PA 3.2 (Distances Between Observations; Ames housing and College Scorecard data). PA student/solution copies are built by `tools/build_week3_pas.py`
  - `*-extended.ipynb` in both directories — fully worked, step-by-step walkthroughs of Topic 3.1, PA 3.1, and PA 3.2 (the colour-coded tables are on the interactive page), built by `tools/build_week3_extended.py`
- `assignments/practice_activities/week_4/`
  - Topic 4.1 (Text as Data: bag of words, TF, TF-IDF, cosine distance) and Topic 4.2 (Strings and Regular Expressions) — 20-minute openers built by `tools/build_week4_topics.py`; Topic 4.1 pairs with PA 4.1, Topic 4.2 with PA 4.2
...
`-extended.ipynb` — executed walkthrough with reasoning for every step and before/after tables (no student version; listed as an *Extended* row on the site).
